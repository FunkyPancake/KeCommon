...
bool LpSpiRtos::Transfer(const uint8_t* txData, uint8_t* rxData, const uint8_t len) const
{
    lpspi_transfer_t data = {
        const_cast<uint8_t*>(txData), rxData, len,
        _flags
    };
    return LPSPI_RTOS_Transfer(this->_handle, &data) == kStatus_Success;
}

bool LpSpiRtos::readBytes(const uint16_t size, std::vector<uint8_t>& data)
{
    uint8_t txBuf[size];
    uint8_t rxBuf[size];
...
uint32_t LpSpiRtos::MapCs(const CsPin cs)
{
